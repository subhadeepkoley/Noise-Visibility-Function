function NVFimage = nvf(image, varargin)
%NVF Generate Noise Visibility Map of an image
...
% Define variance function handle
fun = @(x) var(x(:));

% Compute standard deviation image
sdImage = nlfilter(image, [window, window], fun);

% Calculate theta
theta = D./max(sdImage(:));

% Calculate NVF image
NVFimage = 1./(1+(theta*sdImage));

% Rescale in [0, 1]
NVFimage = rescale(NVFimage);
end